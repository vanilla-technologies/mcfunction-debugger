...
execute unless score @s -ns-_id matches 0 unless entity @e[tag=!-ns-_context,tag=-ns-_tmp] run tellraw @a [{"text":"[Error]","color":"red"},{"text":" Selected entity was killed!\n Start a new debugging session with '/function debug:<your_namespace>/<your_function>'","color":"white"}]
execute unless score @s -ns-_id matches 0 unless entity @e[tag=!-ns-_context,tag=-ns-_tmp] run function -ns-:clean_up
