...
execute unless entity @e[type=area_effect_cloud,tag=-ns-_tmp] as @e[type=area_effect_cloud,tag=-ns-_function_call,tag=-ns-_active] if score @s -ns-_depth = current -ns-_depth run function -ns-:-orig_ns-/-orig/fn-/return_self
