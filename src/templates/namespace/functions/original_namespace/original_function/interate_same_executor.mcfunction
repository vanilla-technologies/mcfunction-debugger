...
execute if score breakpoint namespace_global matches 0 as @e[type=area_effect_cloud,tag=namespace_current] if score @s namespace_depth = current namespace_depth run kill @s

execute if score breakpoint namespace_global matches 0 as @e[type=area_effect_cloud,tag=namespace_selected_entity_marker] if score @s namespace_depth = current namespace_depth run tag @s add namespace_tmp
execute if score breakpoint namespace_global matches 0 if entity @e[type=area_effect_cloud,tag=namespace_tmp] run function namespace:original_namespace/original_function/iterate_same_executor
